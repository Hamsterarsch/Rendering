...
namespace RHA::DX12
{
	...
	void CmdListImpl::RecordSetComputeConstants
	(
		const unsigned parameterIndex,
		const unsigned numConstants,
		const unsigned &constantData,
	...
	{
		glist->SetComputeRoot32BitConstants(parameterIndex, numConstants, &constantData, constantOffsetIntoData);
	...
	void CmdListImpl::RecordSetVertexBuffer(const D3D12_VERTEX_BUFFER_VIEW &view)
	{
		glist->IASetVertexBuffers(0, 1, &view);
		
	}


	
	void CmdListImpl::RecordSetIndexBuffer(const D3D12_INDEX_BUFFER_VIEW &view)
	{
		glist->IASetIndexBuffer(&view);
